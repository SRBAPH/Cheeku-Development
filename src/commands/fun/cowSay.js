module.exports = {
  name: "cowsay",
  description: "Make a cow say anything!",
  aliases: ["cs", "cows", "cowsa", "csay"],
  run: async (client, message, args) => {
    let cowAscii = "\\  ^__^\n \\ (oo)\\_______\n   (__)\\        )\\/\\\n       ||----w |\n       ||     ||";

    if (!args[0]) return message.reply("LOL! You have to specify an argument!");

    let text = args.join(" ");

    message.reply(`\`\`\`${makeSpeech(text, cowAscii)}\`\`\``);
  },
};
